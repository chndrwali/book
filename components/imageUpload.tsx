...
const ImageUpload = ({ onFileChange }: { onFileChange: (filePath: string) => void }) => {
  ...
  const onError = (error: any) => {
    ...
    toast({
      ...
      description: `Gambar gagal diunggah, coba lagi!`,
      variant: 'destructive',
    });
  };
  const onSuccess = (res: any) => {
    ...
    toast({
      ...
      description: `${res.filePath} berhasil diunggah`,
    });
  };
  ...
  return (
    <ImageKitProvider publicKey={publicKey} urlEndpoint={urlEndpoint} authenticator={authenticator}>
      ...
      >
        ...
        <p className="text-base text-light-100">Unggah file</p>
        {file && <p className="uploade-filename">{file.filePath}</p>}
      </button>
